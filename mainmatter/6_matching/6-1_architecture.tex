...
Indeed, to understand the detailed functionality of the proposed personal datastores server, \beatriz{Figure XX} presents a sequence
diagram to demonstrate how its containers, and more specifically its components, interact with each other when a data requester uses a app to petition a certain type of data to be used for a particular purpose.

When data requesters want to have access to a certain resource or data type, they can either use an app to do it, as in \beatriz{Figure XX}, or directly do a request through the Web API.
...
If a previously-given authorisation is still valid, then access to data can be given, however, in \beatriz{Figure XX}, this authorisation does not exist, a fact which could also be kept in an access control log.
...
In case the result is negative, access to data is prohibited, contrarily to the scenario depicted in \beatriz{Figure XX}.
