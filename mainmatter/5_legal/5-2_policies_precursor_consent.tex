...
% \replaced{However, Solid is d}{D}ifferent from P3P, \replaced{because it}{Solid} provides users with a \replaced{decentralized}{decentralised} storage solution for their personal data, with a permission-based access control mechanism, i.e., only with the presence of an \replaced{authorization}{authorisation} stored in the Solid Pod can an application or user access data stored in that same Pod. In addition, in a \replaced{decentralized}{decentralised} environment such as Solid, there is no need to transfer data, as access can be provided to any web user or service, removing the need for services to store or keep copies of the users’ data, through Solid’s \replaced{authorization}{authorisation} and authentication mechanisms.
% These mechanisms can also be used to keep records of access\added{ to} and usage of data, which then can be used to check whether web services are doing what they state in their policies and nothing more---one of the main issues that contributed to the failure of P3P was the lack of enforcement of web services policies (as cited in \added{the }P3P specification ``no enforcement action followed when a site's policy expressed in P3P failed to reflect their actual privacy practices'').
% \added{As such, these mechanisms also allow users to have more transparency and control over how their data are being used, which comes as an improvement over P3P’s lack of consistency between human and machine-readable data handling policies that did not have such controls. In addition, as previously stated in Section \ref{sec:other}, there is research being conducted on the implementation of usage control mechanisms for Solid.}